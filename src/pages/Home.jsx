...
const BlogSlider = () => {
    ...
    const fetchPosts = async () => {
      try {
        const res = await fetch(
          'https://thealcworld.in/store/wp-json/wp/v2/posts?_embed&per_page=6'
        );
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    const slideCount = Math.ceil(posts.length / 3);
    const interval = setInterval(() => {
  ...
  return (
    <div className="relative overflow-hidden">
      <div
        ...
        style={{ width: `${slides.length * 100}%`, transform: `translateX(-${current * 100}%)` }}
      >
            ...
            {group.map((post) => {
              ...
              return (
                <Link
                  key={post.id}
                  to={`/blog/${post.id}`}
                  className="w-full lg:w-1/3 px-3 flex-shrink-0"
                >
